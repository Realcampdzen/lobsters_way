...
openclaw cron add \
  ...
  --cron "0 9 * * *" \
  --message "Прочитай HEARTBEAT.md. Проверь ТГ-канал «Реальный Лагерь» на новые посты. Если есть посты без автокоммента — сгенерируй коммент по skills/tg-autocomment/SKILL.md, оцени по config/scoring-policy.json. Автопубликация только при score >= 7 и PASS от judge. Результат запиши в memory/YYYY-MM-DD.md. Если ничего нового — запиши 'SMM check: ничего нового'." \
  --session isolated
```
...
openclaw cron add \
  ...
  --cron "0 0 * * 3" \
  --message "Прочитай memory/YYYY-MM-DD.md за последнюю неделю. Обнови MEMORY.md значимыми инсайтами. Проверь mistakes.md — есть ли повторяющиеся паттерны? Если да — добавь правило в personality/anti_patterns.md. Сверь scoring-log.md с config/scoring-policy.json и не меняй threshold без отдельного решения человека." \
  --session isolated \
